...
void ST7735_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* data)
{
    ...
    if((y + h - 1) >= _height)
    	tmp_h = _height - y;
    uint16_t *tmp = malloc(w * h * sizeof(uint16_t));
    for(int i = 0; i < tmp_w; i++){
    	for(int j = 0; j < tmp_h; j++){
    		tmp[i + j * tmp_w] = data[i + j * w];
    	}
    }

    TFT_CS_L();
    ST7735_SetAddressWindow(x, y, x + tmp_w - 1, y + tmp_h - 1);
    ST7735_WriteData((uint8_t*)tmp, sizeof(uint16_t) * tmp_w * tmp_h);
    free(tmp);
    TFT_CS_H();
}
